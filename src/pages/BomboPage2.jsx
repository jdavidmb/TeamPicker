...
const BomboPage2 = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/participants')
      .then(res => res.json())
      .then(data => {
        const bombo1Participants = data.filter(participant => participant.bombo === 3);
  ...
  const handleContinue = () => {
    if (selected) {
      fetch(`http://localhost:5000/api/sorteo/equipo/${equipo}`)
        ...
        .then(data => {
          const bombo2Members = data.filter(member => member.bombo === 3);
          console.log(data)
  ...
  return (
    <div className="p-4 min-h-screen bg-gradient-to-tr from-black via-indigo-900 to-purple-900 text-white">
      <h2 className="text-2xl font-bold mb-4 text-center">Sorteo de Equipos: Seguidor</h2>
