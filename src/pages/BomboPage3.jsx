...
const BomboPage3 = () => {
  ...
  useEffect(() => {
    ...
    fetch(import.meta.env.VITE_APP_API_URL + '/api/participants')
      .then(res => res.json())
      .then(data => {
        const bombo1Participants = data.filter(p => p.bombo === 4);
  ...
  const handleContinue = () => {
    ...
    fetch(import.meta.env.VITE_APP_API_URL + `/api/sorteo/equipo/${equipo}`)
      .then(res => res.json())
      .then(data => {
        const bombo2Members = data.filter(m => m.bombo === 4);
              ...
              <div className="mt-6 text-center">
                ...
                <Dialog.Close asChild>
                  <Button
                    ...
                    style={{ fontFamily: "'Orbitron', sans-serif" }}
                  >
                    Finalizar conformación de equipo ➡️
                  </Button>
                </Dialog.Close>
              </div>
            )}
